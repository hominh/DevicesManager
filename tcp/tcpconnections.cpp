...
{
    qDebug() << this << "created TcpConnections";
}
...
{
    if(!sender()) return;
    qDebug() << this << "disconnect socket " << sender();
...
{
    qDebug() << this << "connections start on " << QThread::currentThread();
}
